...
var fetch = function(userId, callback) {
  ...
  getMongoToken(userId, function(err, user) {
    ...
    fetchGithubEvents(github, user.username, from, function(err, events) {
      if (err) return callback(err);
...
var fetchGithubEvents = function(github, username, from, callback) {
  var events = [];
  var currentPage = 1;
  var cont = true;
  async.whilst(
    ...
    function(next) {
      var options = { user: username, page: currentPage, per_page: 99 };
