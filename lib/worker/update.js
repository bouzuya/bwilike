...
var update = function(userId, callback) {
  getUser(userId, function(err, user) {
    ...
    updateTwitterBio(user, function(err, result) {
      if (err) return callback(err);
      var like = format(user.languages);
      twitter.mention(user.username, like, function() {
...
var format = function(languages) {
  return 'like:' + (languages.length > 0 ? (' #' + languages.join(' #')) : '');
};

var formatDescription = function(description, like) {
  var p = /like:.*$/;
